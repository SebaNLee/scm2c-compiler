...
CompilationStatus InstructionLexemeAction(TokenLabel label) {
...
CompilationStatus RegisterLexemeAction() {
	Token * token = createToken(_lexicalAnalyzer, REGISTER);
	token->semanticValue->integer = atoi(token->lexeme + 1); // skips the 'r'
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
